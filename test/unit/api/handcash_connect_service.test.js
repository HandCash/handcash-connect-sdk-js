...
describe('# HandCashConnectService - Unit Tests', () => {
	it('should raise a generic error', async () => {
		...
		return expect(HandCashConnectService.handleApiError(genericError)).to.be.rejectedWith(genericError);
	});
	...
	it('should raise a handcash cloud api error', async () => {
		...
		return expect(HandCashConnectService.handleApiError(handcashApiError)).to.be.rejectedWith(handcashApiError);
	});
